/*
   md.h — progressive non-regex Markdown → HTML (C99, "sane subset")
   ...
   Approach
   --------
   I came up to the ide after checking this post
   (https://dev.to/casualwriter/a-simple-markdown-parser-in-50-lines-of-js-4gpi)
   where someone just applied a series of successive regex substitutions
   to minimaly convert Markdown to HTML.
   Turns out this just "good enough" for my purposes
...
---------------------
I hate parsing Markdown. F*37c Markdown.
...
static void render_header_line(Str *out, const char *line, const char *eol)
{
  ...
  while (slug.len > 0 && slug.buf[slug.len - 1] == '-')
  {
    slug.len -= 1;
    slug.buf[slug.len] = '\0';
  }
  ...
  {
    str_puts(out, " id=\"");
    attr_escape(out, slug.buf, slug.len);
    str_puts(out, "\"");
  }
  str_puts(out, ">");
...
static void render_indented_code(Str *out, const char **pp, const char *block_end)
{
  ...
  if (any)
  {
    ...
    str_putn(out, body.buf, body.len);
...
static void render_text_block(Str *out, const char *begin, const char *end)
{
  ...
  while (p < end)
  {
    ...
    if (!is_blank && *p == '>')
    {
      ...
      {
        str_puts(out, "<p>");
        str_putn(out, para.buf, para.len);
        str_puts(out, "</p>\n");
        para.len = 0;
        in_para = false;
      }
    ...
    str_putn(&para, tmp2.buf, tmp2.len);
    str_free(&tmp2);
