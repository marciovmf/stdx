/*
   markdown.h — C99 progressive non-regex Markdown to HTML 
...
/**
 * @brief Converts a markdown to html.
 * @param markdown A buffer with markdown to be converted to html
 * @return A malloc'ed buffer containing HTML generated from the input buffer
 */
char* md_to_html(const char *markdown, size_t len);

/**
 * @brief Frees the buffer allocated by md_to_html()
 */
void md_free(char *p);
